...
class CompositeTask(BehaviorTask):
    """
    ...
    def __init__(self, *args, config, dataset=None, **kwargs):
        ...
        self._cur_task: BehaviorTask = None
        self._cached_tasks: Dict[str, BehaviorTask] = {}
        ...
        self._inferred_cur_task: Optional[BehaviorTask] = None
    ...
    def get_inferrred_node_task(self) -> BehaviorTask:
        if not self._config.USING_SUBTASKS:
            raise ValueError(
                "Cannot get inferred sub-task when task is not configured to use sub-tasks. See `TASK.USING_SUBTASKS` key."
            )
    ...
    @property
    def forced_node_task(self) -> BehaviorTask:
        """
