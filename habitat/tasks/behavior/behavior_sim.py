...
class BehaviorSim(HabitatSim):
    ...
    def __init__(self, config: Config):
        ...
        self.grasp_mgr: BehaviorGraspManager = BehaviorGraspManager(
    ...
    def reconfigure(self, config: Config):
        ...
        if new_scene:
            ...
            if self.robot.sim_obj is not None and self.robot.sim_obj.is_alive:
                aom.remove_object_by_id(self.robot.sim_obj.object_id)
        ...
        obj_names = [x["object_name"] for x in ep_info["objects"]]
        ...
        # TODO: add articulated object states init
        ...
        print("Agent Start: ", self.habitat_config.AGENT_0.START_POSITION)
        ...
        if self.habitat_config.AGENT_0.IS_SET_START_STATE:
            ...
            print("Setting agent start: ", self.robot.base_pos)
        ...
        # no need to load navmesh (not created for iG scenes anyways). recomputed after loading ojects
        if new_scene or should_add_objects:
            self._load_navmesh()
    ...
    def _load_navmesh(self):
        ...
        # recompute navmesh after adding objects
    ...
    def _add_objs(self, ep_info: Config, should_add_objects: bool) -> None:
        ...
        for i, obj in enumerate(ep_info["objects"]):
            object_template = obj["object_template"]
            object_pos = obj["start_position"]
            object_rot = obj["start_rotation"]
            fixed = obj["fixed"]
            ...
            ao.root_angular_velocity = mn.Vector3.zero_init()
            ao.root_linear_velocity = mn.Vector3.zero_init()

            if should_add_objects:
                self.scene_obj_ids.append(ao.object_id)
    ...
    def get_scene_pos(self) -> np.ndarray:
        ...
        return np.array(
            [
                aom.get_object_by_id(idx).translation
